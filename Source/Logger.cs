    ...
    static class Logger
    {
        ...
        public static void dbgLog(string sText, Exception ex = null, bool bDumpStack = false, bool bNoIncMethod = false)
        {
            try
            {
                ...
                string sPrefix = string.Concat("[", forceResName.MOD_LOG_PREFIX);
                ...
                else
                {
                    ...
                    // to our prefix so you know wtf method triggered your error. ie "[ModPrefixname:forceResClassName.TheMethodThatCausedError]:"
                ...
                if (forceResName.config != null && forceResName.config.UseCustomLogFile == true)
                {
                    string strPath = System.IO.Directory.Exists(Path.GetDirectoryName(forceResName.config.CustomLogFilePath)) ? forceResName.config.CustomLogFilePath.ToString() : Path.Combine(DataLocation.executableDirectory.ToString(), forceResName.config.CustomLogFilePath);
            ...
            catch (Exception Exp)
            {
                ...
                Debug.Log(string.Concat(forceRes.forceResName.MOD_LOG_PREFIX + " Error in log attempt!  ", Exp.Message.ToString()));
            }
            logSB.Length = 0;
